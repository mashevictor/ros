...
function installKinetic()
{
    ...
    while [ $? -ne 0 ]
    do
        echo -e "${red}<sudo rosde init> command invoke error, now will delete /etc/ros/rosdep/sources.list.d/20-default.list file retry ...${normal}"
        sudo rm /etc/ros/rosdep/sources.list.d/20-default.list
        echo -e "${red}now wait 10 seconds to retry...${normal}"
        sleep 10
        sudo rosdep init
    done
